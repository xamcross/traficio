...
4. From `backend/`:
   ...
   ```
5. Check: open `https://traficio.com/login` in a private window → **Continue with
...
### 8. Tell Google the site moved · [C] · 10 minutes

The apex move is **done** (2026-08-23). `https://traficio.com` now serves the whole site.
`www.traficio.com` and `app.traficio.com` return 301 to it, and keep the path and the query
string. Your Search Console property, though, still names the old host.

1. Open Google Search Console. Add a new property for `https://traficio.com`. Use the
   **URL prefix** type.
2. Verify it. The DNS TXT record `google-site-verification=...` is already on the apex, so
   the **Domain name provider** method should pass at once.
3. In the new property open **Sitemaps** and submit `sitemap.xml`.
4. Keep the old `https://app.traficio.com` property. Do **not** delete it. Google reads the
   301 responses through it and moves the signals across. Google's *Change of address* tool
   does not apply here, because both hosts sit on one domain.
5. In a few days confirm the apex URLs appear under **Pages** in the new property.
...
After you finish item 8, open Google Search Console → **Pages** on the
`https://traficio.com` property in a few days. Confirm the 10 URLs move from "Discovered"
to "Indexed". A page that stays out carries a
