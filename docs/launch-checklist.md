...
- [~] 2.3 Database user `geostrategy` with `readWrite` on `geostrategy`. Copy the
      autogenerated password before you click "Add User". Atlas does not show it again.
      If you lost it: Database Access → Edit → Edit Password.
- [ ] 2.4 Read the connection string: Clusters → Connect → Drivers. Append
      `/?maxPoolSize=50&minPoolSize=5`. You need it in step 4.2.
